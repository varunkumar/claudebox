...
`_broadcaster_loop` in `server.py` runs on a background thread. It wakes on `_broadcast_event` (debounced) or after `BROADCASTER_DEBOUNCE_S` seconds. When no sessions are active and nothing triggered the event, it skips the push — no point hammering K10 when Claude Code is idle.
